...
.text
	...
	# laço de escrita dos valores (se repete 10 vezes)
	addu 	$s0, $zero, $zero 	# contador
	la	$s7, arr		# ponteiro para o 1º elemento do array
...
FOR1: 	slti 	$t0, $s0, 10 		# s0<10 -> t0 = 1
	...
	sw	$s1, 0($s7)		# salvar o valor lido na posição do vetor apontado por s7
	addiu 	$s7, $s7, 4		# aponta para a proxima prosição -> s7 += 4 
...
SAI1: 	
	...
	# laço de leitura de valores (se repete 10 vezes)
	addu 	$s0, $zero, $zero 	# contador
	la	$s7, arr		# ponteiro para o 1º elemento do array
...
FOR2: 	slti 	$t0, $s0, 10 		# s0<10 -> t0 = 1
	...
	addiu 	$s7, $s7, 4		# aponta para a proxima posição 
	# calcula soma 
